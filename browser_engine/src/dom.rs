...
impl fmt::Debug for NodeType {
    fn fmt(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            NodeType::Text(ref t ) | NodeType::Comment(ref t) => write!(fmt, "{}", t),
            NodeType::Element(ref e) => write!(fmt, "{:?}", e),
        }
    }
}
...
impl fmt::Debug for ElementData {
    fn fmt(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
        ...
        write!(fmt, "<{},{}>", self.tag_name, attributes_string)
    }
}
